/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:56 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/QuickLook/iWork.qlgenerator/Contents/Frameworks/KeynoteQuicklook.framework/Versions/A/KeynoteQuicklook
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <KeynoteQuicklook/KeynoteQuicklook-Structs.h>
@class KNAnimatedSlideView, NSMutableArray, NSMapTable, NSMutableSet, NSArray, KNTransitionRenderer, KNPlaybackSession, NSSet;

@interface KNAnimatedSlideModel : NSObject {

	KNAnimatedSlideView* _ASV;
	NSMutableArray* _buildEvents;
	NSMutableArray* _animatedBuilds;
	NSMapTable* _infoToArrayOfAnimatedBuildsMap;
	NSMapTable* _animatedBuildToRendererMap;
	NSMutableSet* _ambientBuildRenderers;
	char _isMetalSlide;
	NSArray* _infos;
	unsigned long long _numberOfAddedEvents;
	KNTransitionRenderer* _transitionRenderer;
	KNPlaybackSession* _session;

}

@property (nonatomic,readonly) NSArray * infos;                                                              //@synthesize infos=_infos - In the implementation block
@property (nonatomic,readonly) unsigned long long buildEventCount; 
@property (nonatomic,readonly) NSArray * animatedBuilds;                                                     //@synthesize animatedBuilds=_animatedBuilds - In the implementation block
@property (assign,nonatomic) unsigned long long numberOfAddedEvents;                                         //@synthesize numberOfAddedEvents=_numberOfAddedEvents - In the implementation block
@property (getter=isFirstBuildEventAutomatic,nonatomic,readonly) char firstBuildEventAutomatic; 
@property (nonatomic,readonly) KNTransitionRenderer * transitionRenderer;                                    //@synthesize transitionRenderer=_transitionRenderer - In the implementation block
@property (assign,nonatomic,__weak) KNPlaybackSession * session;                                             //@synthesize session=_session - In the implementation block
@property (nonatomic,readonly) NSSet * ambientBuildRenderers;                                                //@synthesize ambientBuildRenderers=_ambientBuildRenderers - In the implementation block
@property (nonatomic,readonly) char isMetalSlide;                                                            //@synthesize isMetalSlide=_isMetalSlide - In the implementation block
-(void)dealloc;
-(id)description;
-(KNPlaybackSession *)session;
-(void)setSession:(KNPlaybackSession *)arg1 ;
-(NSArray *)infos;
-(id)eventsAtIndex:(unsigned long long)arg1 ;
-(KNTransitionRenderer *)transitionRenderer;
-(unsigned long long)buildEventCount;
-(id)initWithBuildChunks:(id)arg1 infos:(id)arg2 session:(id)arg3 animatedSlideView:(id)arg4 ;
-(void)p_checkIfSlideIsMetalCapableWithBuildChunks:(id)arg1 ;
-(void)p_convertToBuildEvents:(id)arg1 ;
-(void)p_insertInitialAmbientAnimations;
-(id)p_newTransition;
-(void)enumerateAnimatedBuildsAndTimeRangesAtIndex:(unsigned long long)arg1 ignoreInitialDelay:(char)arg2 usingBlock:(/*^block*/id)arg3 ;
-(double)initialDelayForEventIndex:(unsigned long long)arg1 ;
-(id)rendererForAnimatedBuild:(id)arg1 ;
-(long long)p_lastEventNumberForInfo:(id)arg1 animationType:(long long)arg2 defaultResult:(long long)arg3 ;
-(NSArray *)animatedBuilds;
-(id)animatedBuildForInfo:(id)arg1 atEvent:(long long)arg2 ;
-(id)p_previousAnimatedBuildOfType:(long long)arg1 forInfo:(id)arg2 priorToBuild:(id)arg3 ;
-(id)previousAnimatedBuildForInfo:(id)arg1 priorToBuild:(id)arg2 ;
-(id)actionEffectForDrawable:(id)arg1 atBeginningOfEvent:(long long)arg2 ;
-(char)p_isInfoInvalidWithImplicitAmbientAnimations:(id)arg1 ;
-(char)p_isEmphasisBuild:(id)arg1 ;
-(id)p_animatedBuildsForInfo:(id)arg1 animationType:(long long)arg2 duringEvent:(long long)arg3 time:(double)arg4 ;
-(id)p_createBuildAnimationRecords:(id)arg1 info:(id)arg2 event:(long long)arg3 start:(double)arg4 eventStart:(double)arg5 animateAtEndOfPreviousBuild:(char)arg6 previousAnimatedBuild:(id)arg7 parentBuild:(id)arg8 ;
-(void)p_updateVisibilityOnAnimatedBuild:(id)arg1 ;
-(void)p_sortAnimatedBuilds;
-(void)p_updatePreviousAndFinalAttributes;
-(void)p_setRenderer:(id)arg1 forAnimatedBuild:(id)arg2 ;
-(void)p_addAmbientBuildRenderer:(id)arg1 ;
-(Class)p_rendererClassForEffect:(id)arg1 animationType:(long long)arg2 drawable:(id)arg3 parentBuild:(id)arg4 effectClass:(Class*)arg5 ;
-(Class)p_effectClassForEffect:(id)arg1 animationType:(long long)arg2 hasParentBuild:(char)arg3 ;
-(Class)p_rendererClassForBuildChunk:(id)arg1 parentBuild:(id)arg2 effectClass:(Class*)arg3 ;
-(id)p_applyThemeCurvesToBuildAttributes:(id)arg1 ;
-(id)p_newImplicitAmbientBuildRendererWithDrawable:(id)arg1 stageIndex:(long long)arg2 buildChunk:(id)arg3 startTime:(double)arg4 eventStartTime:(double)arg5 event:(long long)arg6 animateAtEndOfPreviousBuild:(char)arg7 ;
-(char)p_shouldAnimateImplicitAmbientAnimatingDrawableAtFirstEvent:(id)arg1 ;
-(long long)inEventForInfo:(id)arg1 ;
-(long long)outEventForInfo:(id)arg1 ;
-(id)p_getCurrentDocumentRoot;
-(void)enumerateAnimatedBuildsAndTimeRangesAtIndex:(unsigned long long)arg1 usingBlock:(/*^block*/id)arg2 ;
-(char)info:(id)arg1 isVisibleDuringEvent:(long long)arg2 ;
-(id)buildChunksForEventRange:(NSRange)arg1 ;
-(double)durationForEventIndexes:(id)arg1 ;
-(id)firstAnimatedBuildForInfo:(id)arg1 ;
-(id)lastAnimatedBuildForInfo:(id)arg1 ;
-(id)animatedBuildForInfo:(id)arg1 aroundEvent:(long long)arg2 isAtEndOfBuild:(char*)arg3 ;
-(id)previousBuildInForInfo:(id)arg1 priorToBuild:(id)arg2 ;
-(id)previousContentBuildForInfo:(id)arg1 priorToBuild:(id)arg2 ;
-(id)previousRendererBeforeRenderer:(id)arg1 ;
-(id)nextAnimatedBuildForInfo:(id)arg1 afterBuild:(id)arg2 ;
-(id)nextRendererAfterRenderer:(id)arg1 ;
-(id)sortRenderers:(id)arg1 ;
-(id)finalActionEffectForDrawable:(id)arg1 ;
-(double)maxScaleFactorForDrawable:(id)arg1 ;
-(char)isFirstBuildEventAutomatic;
-(id)p_animatedBuildForInfo:(id)arg1 event:(long long)arg2 buildIn:(char)arg3 ;
-(NSSet *)ambientBuildRenderers;
-(unsigned long long)numberOfAddedEvents;
-(void)setNumberOfAddedEvents:(unsigned long long)arg1 ;
-(char)isMetalSlide;
@end

