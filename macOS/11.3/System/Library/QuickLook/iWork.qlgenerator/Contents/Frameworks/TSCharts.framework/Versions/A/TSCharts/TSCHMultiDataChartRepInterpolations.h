/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:57 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/QuickLook/iWork.qlgenerator/Contents/Frameworks/TSCharts.framework/Versions/A/TSCharts
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class CAMediaTimingFunction;

@interface TSCHMultiDataChartRepInterpolations : NSObject {

	CAMediaTimingFunction* _overallTimingFunction;
	CAMediaTimingFunction* _individualTimingFunction;

}

@property (nonatomic,readonly) CAMediaTimingFunction * overall;                 //@synthesize overallTimingFunction=_overallTimingFunction - In the implementation block
@property (nonatomic,readonly) CAMediaTimingFunction * individual;              //@synthesize individualTimingFunction=_individualTimingFunction - In the implementation block
+(id)interpolationsWithOverallTimingFunction:(id)arg1 individualTimingFunction:(id)arg2 ;
-(CAMediaTimingFunction *)overall;
-(CAMediaTimingFunction *)individual;
-(id)initWithOverallTimingFunction:(id)arg1 individualTimingFunction:(id)arg2 ;
@end

