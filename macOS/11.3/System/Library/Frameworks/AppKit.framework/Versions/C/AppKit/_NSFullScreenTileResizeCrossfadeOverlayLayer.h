/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:50:57 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/Frameworks/AppKit.framework/Versions/C/AppKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <AppKit/AppKit-Structs.h>
#import <QuartzCore/CALayer.h>

@class _NSFullScreenTileResizeCrossfadeOverlayWindow, CALayer;

@interface _NSFullScreenTileResizeCrossfadeOverlayLayer : CALayer {

	unsigned long long _tileID;
	unsigned long long _parentSpaceID;
	_NSFullScreenTileResizeCrossfadeOverlayWindow* _window;
	char _active;
	CALayer* _windowContentLayer;
	char _includeBorder;
	CGRect _tileFrame;

}
-(void)dealloc;
-(void)resizeForTileFrame:(CGRect)arg1 ;
-(id)initWithTileID:(unsigned long long)arg1 parentSpaceID:(unsigned long long)arg2 tileFrame:(CGRect)arg3 includeBorder:(char)arg4 window:(id)arg5 ;
-(void)setWindowContents:(id)arg1 ;
-(void)addWindowsWithIDs:(id)arg1 ;
-(unsigned long long)tileID;
-(CGRect)tileFrame;
@end

