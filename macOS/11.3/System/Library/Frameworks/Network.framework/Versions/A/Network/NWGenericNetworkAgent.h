/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:51:03 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/Frameworks/Network.framework/Versions/A/Network
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Network/Network-Structs.h>
#import <libobjc.A.dylib/NWPrettyDescription.h>

@class NSString, NSUUID;

@interface NWGenericNetworkAgent : NSObject <NWPrettyDescription> {

	netagent* _internalNetagent;

}

@property (assign) netagent* internalNetagent;                                             //@synthesize internalNetagent=_internalNetagent - In the implementation block
@property (nonatomic,copy,readonly) NSString * privateDescription; 
@property (nonatomic,readonly) NSString * agentDomain; 
@property (nonatomic,readonly) NSString * agentType; 
@property (nonatomic,readonly) NSString * agentDescription; 
@property (nonatomic,readonly) NSUUID * agentUUID; 
@property (getter=isActive,nonatomic,readonly) char active; 
@property (getter=isKernelActivated,nonatomic,readonly) char kernelActivated; 
@property (getter=isUserActivated,nonatomic,readonly) char userActivated; 
@property (getter=isVoluntary,nonatomic,readonly) char voluntary; 
@property (getter=isSpecificUseOnly,nonatomic,readonly) char specificUseOnly; 
@property (getter=isNetworkProvider,nonatomic,readonly) char networkProvider; 
-(id)descriptionWithIndent:(int)arg1 showFullContent:(char)arg2 ;
-(id)initWithKernelAgent:(const netagent*)arg1 ;
-(char)supportsBrowse;
-(char)supportsResolve;
-(netagent*)internalNetagent;
-(void)setInternalNetagent:(netagent*)arg1 ;
-(void)dealloc;
-(id)description;
-(char)isActive;
-(NSString *)privateDescription;
-(NSString *)agentDomain;
-(NSString *)agentType;
-(NSString *)agentDescription;
-(NSUUID *)agentUUID;
-(char)isKernelActivated;
-(char)isUserActivated;
-(char)isVoluntary;
-(char)isSpecificUseOnly;
-(char)isNetworkProvider;
-(char)isNexusProvider;
-(char)requiresAssert;
@end

