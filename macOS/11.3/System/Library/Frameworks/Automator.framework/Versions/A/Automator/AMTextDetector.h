/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:43 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/Frameworks/Automator.framework/Versions/A/Automator
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class NSArray;

@interface AMTextDetector : NSObject {

	NSArray* _typeDictionaries;

}

@property (retain) NSArray * _typeDictionaries;                                 //@synthesize typeDictionaries=_typeDictionaries - In the implementation block
@property (nonatomic,readonly) NSArray * supportedTypeIdentifiers; 
+(id)sharedTextDetector;
-(id)init;
-(NSArray *)supportedTypeIdentifiers;
-(/*^block*/id)_emailAddressHandler;
-(void)set_typeDictionaries:(NSArray *)arg1 ;
-(NSArray *)_typeDictionaries;
-(id)_typeDictionaryForIdentifier:(id)arg1 ;
-(void)_buildTypeDictionaries;
-(id)localizedNameForTypeIdentifier:(id)arg1 ;
-(id)matchesInString:(id)arg1 withType:(id)arg2 error:(id*)arg3 ;
@end

