/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:51:54 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/Frameworks/PencilKit.framework/Versions/A/PencilKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <PencilKit/PKUndoCommand.h>

@class NSArray;

@interface PKModifyStrokesCommand : PKUndoCommand {

	char _hide;
	NSArray* _strokes;

}

@property (nonatomic,readonly) NSArray * strokes;              //@synthesize strokes=_strokes - In the implementation block
@property (nonatomic,readonly) char hide;                      //@synthesize hide=_hide - In the implementation block
+(id)commandForMakingStrokesVisible:(id)arg1 drawing:(id)arg2 hiding:(char)arg3 ;
+(id)commandForMakingStrokeVisible:(id)arg1 drawing:(id)arg2 hiding:(char)arg3 ;
+(id)commandForErasingAllStrokesInDrawing:(id)arg1 ;
+(id)commandForErasingStrokes:(id)arg1 drawing:(id)arg2 ;
-(id)description;
-(char)hide;
-(NSArray *)strokes;
-(void)applyToDrawing:(id)arg1 ;
-(id)invertedInDrawing:(id)arg1 ;
-(id)initWithStrokes:(id)arg1 drawingUUID:(id)arg2 actionName:(id)arg3 hiding:(char)arg4 ;
@end

