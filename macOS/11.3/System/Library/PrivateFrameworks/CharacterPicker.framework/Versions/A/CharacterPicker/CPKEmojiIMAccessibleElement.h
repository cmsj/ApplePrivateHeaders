/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:05 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/CharacterPicker.framework/Versions/A/CharacterPicker
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <CharacterPicker/CharacterPicker-Structs.h>
#import <AppKit/NSAccessibilityElement.h>

@class EMIMDFREmojiListMatrixView;

@interface CPKEmojiIMAccessibleElement : NSAccessibilityElement {

	unsigned long long _index;
	EMIMDFREmojiListMatrixView* _listView;

}

@property (assign) unsigned long long index;                           //@synthesize index=_index - In the implementation block
@property (assign) EMIMDFREmojiListMatrixView * listView;              //@synthesize listView=_listView - In the implementation block
-(unsigned long long)index;
-(void)setIndex:(unsigned long long)arg1 ;
-(CGRect)accessibilityFrame;
-(id)initWithIndex:(unsigned long long)arg1 listView:(id)arg2 ;
-(void)setListView:(EMIMDFREmojiListMatrixView *)arg1 ;
-(EMIMDFREmojiListMatrixView *)listView;
@end

