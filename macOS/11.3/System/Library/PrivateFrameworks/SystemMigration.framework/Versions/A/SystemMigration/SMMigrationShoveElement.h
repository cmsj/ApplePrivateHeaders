/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:38 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/SystemMigration.framework/Versions/A/SystemMigration
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <SystemMigration/SMMigrationEngineElement.h>

@protocol SMShoveElementDelegate;
@class NSObject;

@interface SMMigrationShoveElement : SMMigrationEngineElement {

	NSObject*<SMShoveElementDelegate> _delegate;

}

@property (__weak) NSObject*<SMShoveElementDelegate> delegate;              //@synthesize delegate=_delegate - In the implementation block
-(id)run;
-(NSObject*<SMShoveElementDelegate>)delegate;
-(void)setDelegate:(NSObject*<SMShoveElementDelegate>)arg1 ;
-(double)estimatedTimeToComplete;
@end

