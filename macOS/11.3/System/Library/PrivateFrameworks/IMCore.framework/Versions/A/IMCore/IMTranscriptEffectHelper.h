/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:10 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/IMCore.framework/Versions/A/IMCore
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@interface IMTranscriptEffectHelper : NSObject
+(char)isFeatureEnabled;
+(id)identifierNameMap;
+(char)identifierIsEffectIdentifier:(id)arg1 ;
+(id)allEffectIdentifiers;
+(id)replayStringMap;
+(id)sendWithEffectStringMap;
+(char)effectIdentifierIsFullScreenMoment:(id)arg1 ;
+(char)effectIdentifierIsImpactEffect:(id)arg1 ;
+(id)findIdentifierByMathcingPartialSufix:(id)arg1 ;
+(id)nameForEffectIdentifier:(id)arg1 ;
+(id)replayStringForEffectIdentifier:(id)arg1 ;
+(id)sendWithStringForEffectIdentifier:(id)arg1 ;
+(char)shouldShowReplayButtonForEffectIdentifier:(id)arg1 ;
+(void)setFeatureEnabled:(char)arg1 ;
@end

