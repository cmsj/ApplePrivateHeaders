/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:15 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/Transparency.framework/Versions/A/Transparency
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class TransparencyManagedDataStoreController, NSManagedObjectContext;

@interface TransparencyManagedDataStore : NSObject {

	char _permanentContext;
	TransparencyManagedDataStoreController* _controller;
	NSManagedObjectContext* _context;
	long long _contextRefCount;

}

@property (assign) char permanentContext;                                            //@synthesize permanentContext=_permanentContext - In the implementation block
@property (assign) long long contextRefCount;                                        //@synthesize contextRefCount=_contextRefCount - In the implementation block
@property (__weak) TransparencyManagedDataStoreController * controller;              //@synthesize controller=_controller - In the implementation block
@property (retain) NSManagedObjectContext * context;                                 //@synthesize context=_context - In the implementation block
+(id)serializeLoggableDatas:(id)arg1 ;
+(void)reportCoreDataPersistEventForLocation:(id)arg1 underlyingError:(id)arg2 ;
+(void)reportCoreDataEventForEntity:(id)arg1 write:(char)arg2 code:(long long)arg3 underlyingError:(id)arg4 ;
+(void)reportCoreDataEventForEntity:(id)arg1 hardFailure:(char)arg2 write:(char)arg3 code:(long long)arg4 underlyingError:(id)arg5 ;
+(void)cleanseError:(id*)arg1 ;
+(id)deserializeLoggableDatas:(id)arg1 error:(id*)arg2 ;
-(void)performBlock:(/*^block*/id)arg1 ;
-(NSManagedObjectContext *)context;
-(void)setContext:(NSManagedObjectContext *)arg1 ;
-(id)createRequest;
-(TransparencyManagedDataStoreController *)controller;
-(id)initWithController:(id)arg1 ;
-(void)deleteObject:(id)arg1 ;
-(void)setController:(TransparencyManagedDataStoreController *)arg1 ;
-(void)performBlockAndWait:(/*^block*/id)arg1 ;
-(char)persistWithError:(id*)arg1 ;
-(char)clearStateForApplication:(id)arg1 error:(id*)arg2 ;
-(char)logSTHMetricsForApplication:(id)arg1 error:(id*)arg2 ;
-(char)populateMissingLogHeadHashes:(id*)arg1 ;
-(char)populateExistingRequestsToLookupTable:(id*)arg1 ;
-(id)initWithController:(id)arg1 context:(id)arg2 ;
-(id)createSignedTreeHeadFailure;
-(id)fetchTreeHeadsWithoutHash:(id)arg1 isMapHead:(char)arg2 application:(id)arg3 logBeginTime:(long long)arg4 logType:(long long)arg5 revision:(long long)arg6 error:(id*)arg7 ;
-(void)createKTRequestID:(id)arg1 request:(id)arg2 ;
-(id)requestIds:(id*)arg1 ;
-(id)fetchDownloadRecordById:(id)arg1 error:(id*)arg2 ;
-(void)deleteObjectSet:(id)arg1 ;
-(id)fetchDownloadRecord:(unsigned long long)arg1 application:(id)arg2 error:(id*)arg3 ;
-(id)createDownloadRecord:(unsigned long long)arg1 application:(id)arg2 ;
-(id)downloadRecords:(id*)arg1 ;
-(char)hasPendingDownloadForUUID:(id)arg1 error:(id*)arg2 ;
-(char)setResponse:(id)arg1 downloadId:(id)arg2 error:(id*)arg3 ;
-(char)deleteDownloadRecordById:(id)arg1 error:(id*)arg2 ;
-(char)deleteDownloadRecords:(id)arg1 error:(id*)arg2 ;
-(char)deleteDownloadRecord:(unsigned long long)arg1 application:(id)arg2 error:(id*)arg3 ;
-(char)performAndWaitForDownloadId:(id)arg1 error:(id*)arg2 block:(/*^block*/id)arg3 ;
-(id)createFetchRecord;
-(id)fetchFetchRecordById:(id)arg1 error:(id*)arg2 ;
-(id)createFetchRecordForRequestData:(id)arg1 request:(id)arg2 error:(id*)arg3 ;
-(char)performAndWaitForFetchId:(id)arg1 error:(id*)arg2 block:(/*^block*/id)arg3 ;
-(char)clearState:(id*)arg1 ;
-(id)createSignedMutationTimestamp:(id)arg1 mutationMs:(unsigned long long)arg2 receiptTime:(double)arg3 ;
-(id)unverifiedRevisions:(id)arg1 isMapHead:(char)arg2 inclusion:(char)arg3 logBeginMs:(unsigned long long)arg4 error:(id*)arg5 ;
-(id)latestConsistencyVerifiedTreeHeadRevision:(id)arg1 logBeginMs:(unsigned long long)arg2 error:(id*)arg3 ;
-(char)persistAndRefaultObjects:(id)arg1 error:(id*)arg2 ;
-(void)performForPendingSTHs:(id)arg1 olderThan:(id)arg2 error:(id*)arg3 block:(/*^block*/id)arg4 ;
-(void)performForSTHsWithUnverifiedSignature:(id)arg1 error:(id*)arg2 block:(/*^block*/id)arg3 ;
-(void)garbageCollectSTHs:(id)arg1 logBeginMs:(unsigned long long)arg2 olderThan:(id)arg3 error:(id*)arg4 ;
-(void)performForSTHs:(id)arg1 isMapHead:(char)arg2 revision:(id)arg3 error:(id*)arg4 block:(/*^block*/id)arg5 ;
-(char)haveTreeHead:(id)arg1 isMapHead:(char)arg2 application:(id)arg3 logBeginTime:(long long)arg4 logType:(long long)arg5 revision:(long long)arg6 error:(id*)arg7 ;
-(id)createTreeHead;
-(void)performOnBatchesOfEntity:(id)arg1 predicate:(id)arg2 enforceMax:(char)arg3 error:(id*)arg4 block:(/*^block*/id)arg5 ;
-(void)gargabeCollectEntity:(id)arg1 predicate:(id)arg2 error:(id*)arg3 ;
-(unsigned long long)countOutstandingSTHsForApplication:(id)arg1 error:(id*)arg2 ;
-(unsigned long long)countTotalSTHsForApplication:(id)arg1 error:(id*)arg2 ;
-(id)createTreeHead:(id)arg1 isMapHead:(char)arg2 application:(id)arg3 logBeginTime:(long long)arg4 logHeadHash:(id)arg5 logType:(long long)arg6 revision:(long long)arg7 ;
-(id)treeHeads:(id*)arg1 ;
-(id)treeHeadsForApplication:(id)arg1 error:(id*)arg2 ;
-(unsigned long long)treeHeadCount:(id*)arg1 ;
-(id)fetchTreeHead:(id)arg1 isMapHead:(char)arg2 application:(id)arg3 logBeginTime:(long long)arg4 logType:(long long)arg5 revision:(long long)arg6 error:(id*)arg7 ;
-(char)deleteSTHs:(id)arg1 logBeginMsLessThan:(unsigned long long)arg2 error:(id*)arg3 ;
-(id)createRequestFailure;
-(void)performForPendingSMTs:(id)arg1 uri:(id)arg2 accountId:(id)arg3 error:(id*)arg4 block:(/*^block*/id)arg5 ;
-(char)persistAndRefaultObject:(id)arg1 error:(id*)arg2 ;
-(void)refaultObject:(id)arg1 ;
-(char)performAndWaitForRequestId:(id)arg1 error:(id*)arg2 block:(/*^block*/id)arg3 ;
-(void)performForRequestsWithPendingResponses:(id)arg1 error:(id*)arg2 block:(/*^block*/id)arg3 ;
-(char)resetRequestsToPending:(id)arg1 error:(id*)arg2 ;
-(char)deleteSMTs:(id)arg1 mutationTimeLessThan:(unsigned long long)arg2 error:(id*)arg3 ;
-(id)signedTreeHeadFailures:(id*)arg1 ;
-(unsigned long long)signedTreeHeadFailureCount:(id*)arg1 ;
-(void)performForRequestsWithPendingSMTs:(id)arg1 error:(id*)arg2 block:(/*^block*/id)arg3 ;
-(void)performForSMTsWithUnverifiedSignature:(id)arg1 error:(id*)arg2 block:(/*^block*/id)arg3 ;
-(id)createSignedMutationTimestampsFailure;
-(id)fetchRequestWithUri:(id)arg1 application:(id)arg2 accountID:(id)arg3 loggableDatas:(id)arg4 youngerThan:(id)arg5 error:(id*)arg6 ;
-(id)createRequestWithUri:(id)arg1 application:(id)arg2 accountID:(id)arg3 serverData:(id)arg4 syncedData:(id)arg5 queryRequest:(id)arg6 queryResponse:(id)arg7 type:(unsigned long long)arg8 clientId:(id)arg9 error:(id*)arg10 ;
-(void)performOnRequestsForPredicate:(id)arg1 enforceMax:(char)arg2 error:(id*)arg3 block:(/*^block*/id)arg4 ;
-(unsigned long long)countOutstandingRequestsForApplication:(id)arg1 error:(id*)arg2 ;
-(unsigned long long)countTotalRequestsForApplication:(id)arg1 error:(id*)arg2 ;
-(id)fetchRequestForUUID:(id)arg1 error:(id*)arg2 ;
-(id)createRequestWithUri:(id)arg1 application:(id)arg2 accountID:(id)arg3 serverData:(id)arg4 type:(unsigned long long)arg5 error:(id*)arg6 ;
-(id)createRequestWithUri:(id)arg1 application:(id)arg2 accountID:(id)arg3 serverData:(id)arg4 queryRequest:(id)arg5 queryResponse:(id)arg6 type:(unsigned long long)arg7 error:(id*)arg8 ;
-(id)createRequestWithUri:(id)arg1 application:(id)arg2 accountID:(id)arg3 serverData:(id)arg4 syncedData:(id)arg5 queryRequest:(id)arg6 queryResponse:(id)arg7 type:(unsigned long long)arg8 error:(id*)arg9 ;
-(id)requests:(id*)arg1 ;
-(unsigned long long)requestCount:(id*)arg1 ;
-(id)fetchRequestsForURI:(id)arg1 error:(id*)arg2 ;
-(id)fetchCompletedRequests:(id)arg1 olderThan:(id)arg2 error:(id*)arg3 ;
-(void)garbageCollectRequests:(id)arg1 olderThan:(id)arg2 error:(id*)arg3 ;
-(char)logRequestMetricsForApplication:(id)arg1 error:(id*)arg2 ;
-(id)requestFailures:(id*)arg1 ;
-(unsigned long long)requestFailureCount:(id*)arg1 ;
-(id)signedMutationTimestampsFailures:(id*)arg1 ;
-(unsigned long long)signedMutationTimestampsFailureCount:(id*)arg1 ;
-(char)logMetricsForApplication:(id)arg1 error:(id*)arg2 ;
-(void)setPermanentContext:(char)arg1 ;
-(void)setContextRefCount:(long long)arg1 ;
-(id)signedMutationTimestamps:(id*)arg1 ;
-(char)logSMTMetricsForApplication:(id)arg1 error:(id*)arg2 ;
-(unsigned long long)signedMutationTimestampCount:(id*)arg1 ;
-(char)permanentContext;
-(long long)contextRefCount;
-(id)retainContext;
-(void)releaseContext;
-(id)copyStatistics:(id*)arg1 ;
-(unsigned long long)countOutstandingSMTsForApplication:(id)arg1 error:(id*)arg2 ;
-(unsigned long long)countTotalSMTsForApplication:(id)arg1 error:(id*)arg2 ;
@end

