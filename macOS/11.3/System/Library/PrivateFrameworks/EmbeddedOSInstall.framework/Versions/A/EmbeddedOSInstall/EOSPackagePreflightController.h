/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:03 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/EmbeddedOSInstall.framework/Versions/A/EmbeddedOSInstall
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@interface EOSPackagePreflightController : NSObject
+(id)sharedController;
-(id)_restoreBundleSpecifiersInPackageSpecifiers:(id)arg1 ;
-(char)_extractRestoreBundleFromPackage:(id)arg1 toDestination:(id)arg2 ;
-(id)_restoreBundleSpecifierFromFirmwareBundleComponent:(id)arg1 packageSpecifier:(id)arg2 ;
-(void)preflightPackageSpecifiers:(id)arg1 request:(id)arg2 tagPrefix:(id)arg3 completionHandler:(/*^block*/id)arg4 ;
@end

