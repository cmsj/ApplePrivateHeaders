/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:04 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/AutoBugCaptureCore.framework/Versions/A/AutoBugCaptureCore
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <libobjc.A.dylib/NSCoding.h>

@class NSMutableDictionary, NSUUID, DiagnosticCaseStorage, NSMutableArray, NSDictionary, DiagnosticCaseManager, NSString;

@interface DiagnosticCase : NSObject <NSCoding> {

	char eventsDirty;
	char payloadsDirty;
	char attachmentsDirty;
	char _allowIPSFileOutput;
	unsigned long long completedDiagnosticActions;
	unsigned long long expectedDiagnosticActions;
	NSMutableDictionary* _requiredActions;
	NSMutableDictionary* _requiredAttachments;
	char _readyToEndSession;
	NSUUID* _caseId;
	DiagnosticCaseStorage* _caseStorage;
	NSMutableArray* _caseReports;
	NSMutableArray* _caseOutlets;
	NSMutableDictionary* _signature;
	NSMutableArray* _events;
	NSMutableArray* _payloads;
	NSMutableArray* _attachments;
	NSDictionary* _actions;
	unsigned long long _caseFlags;
	double _preferredDuration;
	DiagnosticCaseManager* _manager;
	NSMutableDictionary* _attachmentsWithSize;

}

@property (nonatomic,retain) DiagnosticCaseManager * manager;                          //@synthesize manager=_manager - In the implementation block
@property (nonatomic,retain) NSMutableDictionary * attachmentsWithSize;                //@synthesize attachmentsWithSize=_attachmentsWithSize - In the implementation block
@property (nonatomic,readonly) NSUUID * caseId;                                        //@synthesize caseId=_caseId - In the implementation block
@property (nonatomic,readonly) DiagnosticCaseStorage * caseStorage;                    //@synthesize caseStorage=_caseStorage - In the implementation block
@property (nonatomic,readonly) NSMutableDictionary * signature;                        //@synthesize signature=_signature - In the implementation block
@property (nonatomic,readonly) double caseOpenedTime; 
@property (nonatomic,readonly) double caseClosedTime; 
@property (nonatomic,readonly) NSMutableArray * events;                                //@synthesize events=_events - In the implementation block
@property (nonatomic,readonly) NSMutableArray * payloads;                              //@synthesize payloads=_payloads - In the implementation block
@property (nonatomic,readonly) NSMutableArray * attachments;                           //@synthesize attachments=_attachments - In the implementation block
@property (nonatomic,readonly) NSDictionary * actions;                                 //@synthesize actions=_actions - In the implementation block
@property (nonatomic,readonly) NSMutableArray * caseReports;                           //@synthesize caseReports=_caseReports - In the implementation block
@property (nonatomic,readonly) NSMutableArray * caseOutlets;                           //@synthesize caseOutlets=_caseOutlets - In the implementation block
@property (nonatomic,readonly) short caseState; 
@property (nonatomic,readonly) short closureType; 
@property (nonatomic,readonly) short dampeningType; 
@property (getter=isSnapshot,nonatomic,readonly) char snapshot; 
@property (getter=wantsRemote,nonatomic,readonly) char wantsRemote; 
@property (getter=isRemoteTrigger,nonatomic,readonly) char remoteTrigger; 
@property (assign,nonatomic) unsigned long long caseFlags;                             //@synthesize caseFlags=_caseFlags - In the implementation block
@property (assign,nonatomic) double preferredDuration;                                 //@synthesize preferredDuration=_preferredDuration - In the implementation block
@property (nonatomic,readonly) char writeIPSFile; 
@property (assign,nonatomic) char readyToEndSession;                                   //@synthesize readyToEndSession=_readyToEndSession - In the implementation block
@property (nonatomic,readonly) NSDictionary * requiredAttachments;                     //@synthesize requiredAttachments=_requiredAttachments - In the implementation block
@property (nonatomic,readonly) NSString * caseDomain; 
@property (nonatomic,readonly) NSString * caseType; 
@property (nonatomic,readonly) NSString * caseSubType; 
@property (nonatomic,readonly) NSString * caseSubTypeContext; 
@property (nonatomic,readonly) NSString * caseDetectedProcess; 
@property (nonatomic,readonly) NSString * caseEffectiveProcess; 
+(id)descriptionForDampeningType:(short)arg1 ;
+(id)descriptionForClosureType:(short)arg1 ;
+(id)attachmentsFromStringRepresentation:(id)arg1 ;
+(id)generateCaseSummaryFromCaseStorageDictionary:(id)arg1 ;
+(id)_closureTypeString:(short)arg1 ;
+(id)_caseDampeningTypeString:(short)arg1 ;
+(id)_stringRepresentationOfJSONObject:(id)arg1 prettyPrint:(char)arg2 ;
+(id)_arrayObjectRepresentationOfJSONString:(id)arg1 ;
+(id)stringRepresentationOfAttachments:(id)arg1 ;
+(id)signatureForCaseStorage:(id)arg1 ;
+(id)signatureForCaseStorageDictionary:(id)arg1 ;
+(id)generateCaseSummaryDictionaryFromCaseStorage:(id)arg1 ;
-(void)dealloc;
-(id)description;
-(void)encodeWithCoder:(id)arg1 ;
-(id)initWithCoder:(id)arg1 ;
-(DiagnosticCaseManager *)manager;
-(void)setManager:(DiagnosticCaseManager *)arg1 ;
-(id)dictionaryRepresentation;
-(NSMutableArray *)attachments;
-(NSMutableDictionary *)signature;
-(NSMutableArray *)events;
-(NSDictionary *)actions;
-(NSMutableArray *)payloads;
-(void)addEvent:(id)arg1 ;
-(char)isSnapshot;
-(void)addPayload:(id)arg1 ;
-(void)addEvents:(id)arg1 ;
-(char)matchSignature:(id)arg1 ;
-(double)caseOpenedTime;
-(id)deParametersFromPayloads;
-(short)dampeningType;
-(void)setDampeningType:(short)arg1 ;
-(double)caseClosedTime;
-(NSString *)caseDomain;
-(NSString *)caseType;
-(NSString *)caseDetectedProcess;
-(NSString *)caseEffectiveProcess;
-(void)allowIPSFileOutput:(char)arg1 ;
-(NSString *)caseSubType;
-(char)writeIPSFile;
-(NSUUID *)caseId;
-(id)caseDampeningTypeString;
-(id)caseClosureTypeString;
-(void)addRequiredAttachmentType:(id)arg1 pattern:(id)arg2 ;
-(DiagnosticCaseStorage *)caseStorage;
-(id)stringRepresentationOfAttachmentsWithSize;
-(void)prepareForSaving;
-(short)caseState;
-(char)isRemoteTrigger;
-(short)closureType;
-(id)initWithSignature:(id)arg1 flags:(unsigned long long)arg2 events:(id)arg3 payload:(id)arg4 actions:(id)arg5 manager:(id)arg6 ;
-(void)setClosureType:(short)arg1 ;
-(void)setCaseState:(short)arg1 ;
-(id)initWithCaseId:(id)arg1 manager:(id)arg2 ;
-(char)readyToEndSession;
-(double)preferredDuration;
-(void)finishCase;
-(void)reportGeneratorEnded:(id)arg1 reportInfo:(id)arg2 error:(id)arg3 isFinalReportGenerator:(char)arg4 ;
-(void)addRequiredAction:(id)arg1 option:(id)arg2 attachmentType:(id)arg3 pattern:(id)arg4 ;
-(void)setPreferredDuration:(double)arg1 ;
-(void)setReadyToEndSession:(char)arg1 ;
-(char)wantsRemote;
-(id)initWithCaseStorage:(id)arg1 manager:(id)arg2 ;
-(char)_parseSignatureIntoCaseStorage:(id)arg1 ;
-(void)addGroupIdentifierToSignature;
-(char)_parseCaseStorageIntoSignature:(id)arg1 ;
-(void)deserializeDiagnosticCaseStorageEvents;
-(void)deserializeDiagnosticCaseStoragePayloads;
-(void)deserializeDiagnosticCaseStorageAttachments;
-(id)caseStateString:(short)arg1 ;
-(id)_generateCaseSummaryDictionary;
-(void)prepareCase;
-(void)collectLogs;
-(void)noteCaseClosedTime;
-(void)prepareAttachmentsForReporting;
-(void)validateAttachments;
-(void)summarizeCaseReport;
-(void)recordDiagnosticCaseSummary;
-(char)sendReports;
-(void)noteCaseClosedTime:(double)arg1 ;
-(void)noteCaseOpenedTime:(double)arg1 ;
-(void)addEvent:(id)arg1 forced:(char)arg2 ;
-(void)addEvents:(id)arg1 forced:(char)arg2 ;
-(id)stringRepresentationOfEvents;
-(id)eventsFromStringRepresentation;
-(id)stringRepresentationOfPayloads;
-(id)payloadsFromStringRepresentation;
-(id)addSizeInfoForAttachment:(id)arg1 ;
-(void)_updatePayloadDictionaryArray:(char)arg1 ;
-(void)startCollectingDiagnosticExtensions:(id)arg1 ;
-(void)startCollectingAllReports;
-(void)removeRequiredAttachmentType:(id)arg1 pattern:(id)arg2 ;
-(void)_addToAttachments:(id)arg1 ;
-(void)finishedCollectingLogs;
-(NSMutableArray *)caseReports;
-(id)_updatedPayloadPathForOriginalPath:(id)arg1 sandboxExtensionToken:(id)arg2 ;
-(NSMutableDictionary *)attachmentsWithSize;
-(void)setAttachmentsWithSize:(NSMutableDictionary *)arg1 ;
-(id)_caseSummaryFileName;
-(void)fixupCaseSummaryFile:(id)arg1 ;
-(void)serializePayloadsToDiagnosticCaseStorage;
-(void)serializeAttachmentsToDiagnosticCaseStorage;
-(void)serializeEventsToDiagnosticCaseStorage;
-(void)noteCaseOpenedTime;
-(NSString *)caseSubTypeContext;
-(id)caseProcess;
-(short)caseStateFromString:(id)arg1 ;
-(id)stringRepresentationOfSignature;
-(NSMutableArray *)caseOutlets;
-(NSDictionary *)requiredAttachments;
-(unsigned long long)caseFlags;
-(void)setCaseFlags:(unsigned long long)arg1 ;
@end

