/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:46 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/ConsoleKit.framework/Versions/A/ConsoleKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <ConsoleKit/ConsoleKit-Structs.h>
#import <AppKit/NSTableHeaderView.h>

@interface CSKTableHeaderView : NSTableHeaderView {

	char _wantsTopBorder;

}

@property (assign,nonatomic) char wantsTopBorder;              //@synthesize wantsTopBorder=_wantsTopBorder - In the implementation block
-(id)init;
-(id)initWithCoder:(id)arg1 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)drawRect:(CGRect)arg1 ;
-(void)_commonInit;
-(char)wantsTopBorder;
-(void)setWantsTopBorder:(char)arg1 ;
@end

