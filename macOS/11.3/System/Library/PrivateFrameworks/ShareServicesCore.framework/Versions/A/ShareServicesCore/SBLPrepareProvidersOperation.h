/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:51 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/ShareServicesCore.framework/Versions/A/ShareServicesCore
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <ShareServicesCore/SBLProvidersOperation.h>

@class NSArray;

@interface SBLPrepareProvidersOperation : SBLProvidersOperation {

	NSArray* _providersForDownload;

}

@property (nonatomic,readonly) NSArray * providersForDownload;              //@synthesize providersForDownload=_providersForDownload - In the implementation block
-(void)px_start;
-(NSArray *)providersForDownload;
@end

