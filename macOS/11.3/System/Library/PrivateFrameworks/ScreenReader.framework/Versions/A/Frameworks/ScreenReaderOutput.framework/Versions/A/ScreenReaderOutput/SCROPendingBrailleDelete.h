/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:50 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/ScreenReader.framework/Versions/A/Frameworks/ScreenReaderOutput.framework/Versions/A/ScreenReaderOutput
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <ScreenReaderOutput/SCROPendingBrailleEdit.h>

@interface SCROPendingBrailleDelete : SCROPendingBrailleEdit {

	char _isForwardDelete;

}

@property (assign,nonatomic) char isForwardDelete;              //@synthesize isForwardDelete=_isForwardDelete - In the implementation block
-(char)isForwardDelete;
-(void)setIsForwardDelete:(char)arg1 ;
@end

