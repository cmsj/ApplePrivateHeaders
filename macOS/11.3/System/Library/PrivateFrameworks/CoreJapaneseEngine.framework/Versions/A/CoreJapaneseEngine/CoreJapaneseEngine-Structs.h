/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:47 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/CoreJapaneseEngine.framework/Versions/A/CoreJapaneseEngine
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

typedef struct __MecabraContext* MecabraContextRef;

typedef struct JIMDebugSegmentOperationInfo {
	long long lastSegmentOperation;
	long long lastChangedSegmentIndex;
	long long lastAddedLogLength;
} JIMDebugSegmentOperationInfo;

typedef struct __Mecabra* MecabraRef;

typedef struct _NSZone* NSZoneRef;

typedef struct NSRange {
	unsigned long long location;
	unsigned long long length;
} NSRange;

typedef struct __TISInputSource* TISInputSourceRef;

typedef struct CGSize {
	double width;
	double height;
} CGSize;

