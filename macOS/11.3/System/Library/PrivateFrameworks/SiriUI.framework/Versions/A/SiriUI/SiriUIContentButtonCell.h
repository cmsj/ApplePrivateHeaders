/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:32 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/SiriUI.framework/Versions/A/SiriUI
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <SiriUI/SiriUI-Structs.h>
#import <AppKit/NSButtonCell.h>

@interface SiriUIContentButtonCell : NSButtonCell {

	NSEdgeInsets _contentEdgeInsets;

}

@property (assign) NSEdgeInsets contentEdgeInsets;              //@synthesize contentEdgeInsets=_contentEdgeInsets - In the implementation block
-(void)layoutLayerWithFrame:(CGRect)arg1 inView:(id)arg2 ;
-(NSEdgeInsets)contentEdgeInsets;
-(void)setContentEdgeInsets:(NSEdgeInsets)arg1 ;
-(char)_allowsVibrancyForTitleInView:(id)arg1 ;
@end

