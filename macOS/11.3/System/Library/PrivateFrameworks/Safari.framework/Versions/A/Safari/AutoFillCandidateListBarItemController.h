/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:07 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/Safari.framework/Versions/A/Safari
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <libobjc.A.dylib/NSCandidateListTouchBarItemDelegate.h>

@protocol AutoFillCandidateListBarItemControllerDelegate;
@class NSString, NSCandidateListTouchBarItem, NSTouchBarItem;

@interface AutoFillCandidateListBarItemController : NSObject <NSCandidateListTouchBarItemDelegate> {

	NSString* _itemIdentifier;
	NSCandidateListTouchBarItem* _item;
	id<AutoFillCandidateListBarItemControllerDelegate> _delegate;

}

@property (assign,nonatomic,__weak) id<AutoFillCandidateListBarItemControllerDelegate> delegate;              //@synthesize delegate=_delegate - In the implementation block
@property (nonatomic,readonly) NSTouchBarItem * barItem; 
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(id<AutoFillCandidateListBarItemControllerDelegate>)delegate;
-(void)setDelegate:(id<AutoFillCandidateListBarItemControllerDelegate>)arg1 ;
-(id)initWithIdentifier:(id)arg1 ;
-(void)candidateListTouchBarItem:(id)arg1 endSelectingCandidateAtIndex:(long long)arg2 ;
-(NSTouchBarItem *)barItem;
-(void)refreshCandidates;
@end

