/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:51:52 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/MediaStream.framework/Versions/A/MediaStream
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@interface MSBatteryPowerMonitor : NSObject {

	char _isExternalPowerConnected;

}

@property (nonatomic,readonly) char isExternalPowerConnected;              //@synthesize isExternalPowerConnected=_isExternalPowerConnected - In the implementation block
+(id)defaultMonitor;
+(char)isExternalPowerConnected:(double*)arg1 ;
-(double)batteryPercentRemaining;
-(char)isExternalPowerConnected;
@end

