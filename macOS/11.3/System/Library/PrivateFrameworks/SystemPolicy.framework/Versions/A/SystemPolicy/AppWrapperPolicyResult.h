/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:51:51 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/SystemPolicy.framework/Versions/A/SystemPolicy
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@interface AppWrapperPolicyResult : NSObject {

	char isValid;
	char isModified;
	unsigned long long signerCategory;

}

@property (nonatomic,readonly) char isValid; 
@property (nonatomic,readonly) char isModified; 
@property (nonatomic,readonly) unsigned long long signerCategory; 
-(id)description;
-(id)redactedDescription;
-(char)isValid;
-(char)isModified;
-(id)initWithValid:(char)arg1 withSignerCategory:(unsigned long long)arg2 withModified:(char)arg3 ;
-(unsigned long long)signerCategory;
@end

