/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:10 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/PersonalizationPortraitInternals.framework/Versions/A/PersonalizationPortraitInternals
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class NSMutableArray;

@interface PPContactRecordResultBuffer : NSObject {

	char _loadingComplete;
	NSMutableArray* _records;

}

@property (nonatomic,readonly) NSMutableArray * records;              //@synthesize records=_records - In the implementation block
@property (assign,nonatomic) char loadingComplete;                    //@synthesize loadingComplete=_loadingComplete - In the implementation block
-(id)init;
-(NSMutableArray *)records;
-(char)loadingComplete;
-(void)setLoadingComplete:(char)arg1 ;
@end

