/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:55 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/WorkflowKit.framework/Versions/A/WorkflowKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <libobjc.A.dylib/INIntentResponseObserver.h>
#import <libobjc.A.dylib/NSProgressReporting.h>

@protocol WFIntentExecutorDelegate, INCExtensionProxy;
@class NSProgress, INCExtensionConnection, INIntent, NSString;

@interface WFIntentExecutor : NSObject <INIntentResponseObserver, NSProgressReporting> {

	WFIntentExecutor* _strongSelf;
	char _forceExecutionOnPhone;
	char _skipResolveAndConfirm;
	NSProgress* _progress;
	INCExtensionConnection* _connection;
	id<WFIntentExecutorDelegate> _delegate;
	/*^block*/id _completionHandler;
	id<INCExtensionProxy> _getRideStatusUpdatingExtensionProxy;

}

@property (nonatomic,copy,readonly) id completionHandler;                                            //@synthesize completionHandler=_completionHandler - In the implementation block
@property (nonatomic,retain) id<INCExtensionProxy> getRideStatusUpdatingExtensionProxy;              //@synthesize getRideStatusUpdatingExtensionProxy=_getRideStatusUpdatingExtensionProxy - In the implementation block
@property (assign,nonatomic) char forceExecutionOnPhone;                                             //@synthesize forceExecutionOnPhone=_forceExecutionOnPhone - In the implementation block
@property (assign,nonatomic) char skipResolveAndConfirm;                                             //@synthesize skipResolveAndConfirm=_skipResolveAndConfirm - In the implementation block
@property (nonatomic,readonly) INIntent * intent; 
@property (nonatomic,readonly) INCExtensionConnection * connection;                                  //@synthesize connection=_connection - In the implementation block
@property (assign,nonatomic,__weak) id<WFIntentExecutorDelegate> delegate;                           //@synthesize delegate=_delegate - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (readonly) NSProgress * progress;                                                          //@synthesize progress=_progress - In the implementation block
-(id<WFIntentExecutorDelegate>)delegate;
-(void)setDelegate:(id<WFIntentExecutorDelegate>)arg1 ;
-(INCExtensionConnection *)connection;
-(void)failWithError:(id)arg1 ;
-(void)finish;
-(void)cancel;
-(NSProgress *)progress;
-(id)completionHandler;
-(INIntent *)intent;
-(void)startWithCompletionHandler:(/*^block*/id)arg1 ;
-(void)intentResponseDidUpdate:(id)arg1 withSerializedCacheItems:(id)arg2 ;
-(id)initWithIntent:(id)arg1 donateInteraction:(char)arg2 groupIdentifier:(id)arg3 ;
-(void)getDefaultValueForParameterNamed:(id)arg1 completionHandler:(/*^block*/id)arg2 ;
-(id)errorFromResolutionResult:(id)arg1 forSlot:(id)arg2 onIntent:(id)arg3 ;
-(id)errorFromConfirmResponse:(id)arg1 intent:(id)arg2 ;
-(id)errorFromHandleResponse:(id)arg1 intent:(id)arg2 ;
-(void)retainSelf;
-(void)autoreleaseSelf;
-(id)extensionInputItemsWithIntent:(id)arg1 ;
-(void)getDynamicOptionsForParameterNamed:(id)arg1 searchTerm:(id)arg2 completionHandler:(/*^block*/id)arg3 ;
-(void)configureConnection;
-(void)startConnectionWithCompletionHandler:(/*^block*/id)arg1 ;
-(void)resolveIntent:(id)arg1 withExtensionProxy:(id)arg2 ;
-(void)resolveIntentResolutionResults:(id)arg1 slotDescription:(id)arg2 intent:(id)arg3 completion:(/*^block*/id)arg4 ;
-(void)confirmIntent:(id)arg1 withExtensionProxy:(id)arg2 ;
-(void)handleIntent:(id)arg1 withExtensionProxy:(id)arg2 ;
-(void)handleIntent:(id)arg1 withExtensionProxy:(id)arg2 completionHandler:(/*^block*/id)arg3 ;
-(void)updateIntentWithItemToConfirm:(id)arg1 forSlot:(id)arg2 onIntent:(id)arg3 ;
-(void)startReceivingRideStatusUpdates;
-(void)stopReceivingRideStatusUpdates;
-(id)errorFromExtensionError:(id)arg1 ;
-(void)finishWithInteraction:(id)arg1 error:(id)arg2 ;
-(void)finishWithInteraction:(id)arg1 ;
-(char)continueInApp;
-(void)showConfirmationForInteraction:(id)arg1 confirmationRequired:(char)arg2 authenticationRequired:(char)arg3 completionHandler:(/*^block*/id)arg4 ;
-(id<INCExtensionProxy>)getRideStatusUpdatingExtensionProxy;
-(void)setGetRideStatusUpdatingExtensionProxy:(id<INCExtensionProxy>)arg1 ;
-(char)forceExecutionOnPhone;
-(void)setForceExecutionOnPhone:(char)arg1 ;
-(char)skipResolveAndConfirm;
-(void)setSkipResolveAndConfirm:(char)arg1 ;
@end

