/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:28 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/SafariSharedUI.framework/Versions/A/SafariSharedUI
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <SafariSharedUI/SafariSharedUI-Structs.h>
@class NSString, NSDate;

@interface WBSFaviconProviderIconInfo : NSObject {

	char _hasGeneratedResolutions;
	char _isRejectedResource;
	NSString* _pageURLString;
	NSString* _iconURLString;
	NSString* _UUIDString;
	NSDate* _dateAdded;
	CGSize _size;

}

@property (nonatomic,copy,readonly) NSString * pageURLString;              //@synthesize pageURLString=_pageURLString - In the implementation block
@property (nonatomic,copy,readonly) NSString * iconURLString;              //@synthesize iconURLString=_iconURLString - In the implementation block
@property (nonatomic,copy,readonly) NSString * UUIDString;                 //@synthesize UUIDString=_UUIDString - In the implementation block
@property (nonatomic,readonly) NSDate * dateAdded;                         //@synthesize dateAdded=_dateAdded - In the implementation block
@property (nonatomic,readonly) CGSize size;                                //@synthesize size=_size - In the implementation block
@property (nonatomic,readonly) char hasGeneratedResolutions;               //@synthesize hasGeneratedResolutions=_hasGeneratedResolutions - In the implementation block
@property (nonatomic,readonly) char isRejectedResource;                    //@synthesize isRejectedResource=_isRejectedResource - In the implementation block
-(CGSize)size;
-(id)init;
-(NSString *)UUIDString;
-(NSDate *)dateAdded;
-(NSString *)iconURLString;
-(char)isRejectedResource;
-(char)hasGeneratedResolutions;
-(id)initWithPageURLString:(id)arg1 iconURLString:(id)arg2 UUIDString:(id)arg3 dateAdded:(id)arg4 size:(CGSize)arg5 hasGeneratedResolutions:(char)arg6 isRejectedResource:(char)arg7 ;
-(NSString *)pageURLString;
@end

