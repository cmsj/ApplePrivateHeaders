/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, April 28, 2021 at 2:52:25 PM British Summer Time
* Operating System: Version 11.3 (Build 20E232)
* Image Source: /System/Library/PrivateFrameworks/PhotoImaging.framework/Versions/A/PhotoImaging
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <NeutrinoCore/NURuleSystem.h>

@interface PIReframeRules : NURuleSystem

@property (readonly) char isCandidateForReframe; 
@property (readonly) char isCandidateForPerspective; 
@property (readonly) char isCandidateForHorizon; 
+(id)factCandidateForHorizon;
+(id)factCandidateForPerspective;
+(id)factCandidateForReframe;
+(id)factCandidateForStill;
+(id)factCandidateForVideo;
+(id)sharedPregateRules;
+(id)pregateRulesSystemWithConstants:(id)arg1 ;
-(char)isCandidateForReframe;
-(char)isCandidateForPerspective;
-(char)isCandidateForHorizon;
@end

